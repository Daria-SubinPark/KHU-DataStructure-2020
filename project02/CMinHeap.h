...
void CMinHeap<T>::ReheapDown(int iparent, int ibottom)
{
    ...
    if ( leftChild <= ibottom )
    {
        ...
        if ( CHeapBase<T>::m_pHeap [ iparent ] > CHeapBase<T>::m_pHeap [ maxChild ] )
        {
            CHeapBase<T>::Swap ( iparent, maxChild ) ; // 두 데이터의 위치를 바꾼다
...
void CMinHeap<T>::ReheapUp(int iroot, int ibottom)
{
    ...
    if ( ibottom > iroot )
    {
        ...
        {
            CHeapBase<T>::Swap ( iparent, ibottom ) ; // 두 데이터의 위치를 바꾼다
...
void CMinHeap<T>::Delete(T item, bool &found, int iparent)
{
    ...
    if(CHeapBase<T>::m_pHeap[iparent] == item) // 삭제하려는 데이터 발견
    {
        CHeapBase<T>::m_pHeap[iparent] = CHeapBase<T>::m_pHeap[CHeapBase<T>::m_iLastNode-1];
        ReheapDown(iparent, CHeapBase<T>::m_iLastNode-2); // 데이터 정렬
        found = true;
    }
...
void CMinHeap<T>::Retrieve(T &item, bool &found, int iparent)
{
    ...
    if(CHeapBase<T>::m_pHeap[iparent] == item) // 검색하려는 데이터 발견
    {
